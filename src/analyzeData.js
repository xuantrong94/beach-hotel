...
function formatData(items) {
  const getArrayImageUrls = (items) => {
    return items.fields.images.map(
      (image) => image.fields.file.url
    );
  };
  let tempItems = items.map((item) => {
    let id = item.sys.id; // result: int
    let images = getArrayImageUrls(item);
...
function getIdOfFeaturedRooms(data) {
...
console.log(getIdOfFeaturedRooms(formattedData));

function getId(arrItem) {
  return arrItem.map((item) => item.id);
}
